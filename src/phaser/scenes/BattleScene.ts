...
export class BattleScene extends Phaser.Scene {
  ...
  private syncPlayer(state: RunState): void {
    ...
    this.playerSprite.setScale(baseScale * (state.player.berserkTimer > 0 ? 1.14 : 1) * (ultimateActive ? 1.08 : 1));
  ...
  private syncAreas(areas: AreaState[]): void {
    ...
    for (const area of areas) {
      ...
      const isPetalArea = area.vfxKey.includes("petal") || area.vfxKey.includes("allure");
      graphics.clear();
      graphics.fillStyle(color, isPetalArea ? Math.min(0.13, 0.05 + area.ttl * 0.035) : Math.min(0.28, 0.1 + area.ttl * 0.08));
      graphics.fillCircle(area.x, area.y, area.radius);
      graphics.lineStyle(isPetalArea ? 1.5 : 2, color, isPetalArea ? 0.34 : 0.55);
      graphics.strokeCircle(area.x, area.y, area.radius * (0.85 + Math.sin(this.time.now / 80) * 0.05));
      if (isPetalArea) {
        graphics.fillStyle(0xffb7d6, 0.24);
  ...
  private syncCombatEvents(events: CombatEventState[]): void {
    ...
    for (const event of events) {
      ...
      if (event.type === "hit" || event.type === "crit" || event.type === "kill" || event.type === "playerHit") {
        ...
        const isPetalEvent = event.vfxKey.includes("petal") || event.vfxKey.includes("allure");
        graphics.lineStyle(isPetalEvent ? 3 : 5, color, (isPetalEvent ? 0.46 : 0.75) * (1 - progress));
        graphics.strokeCircle(event.x, event.y, radius);
        graphics.lineStyle(2, 0xfff1cf, (isPetalEvent ? 0.22 : 0.35) * (1 - progress));
